...
// Instance the plugin
WPZOOM_Elementor_Form::instance();

/**
 * Class WPZOOM_Elementor_Form
 */
...
class WPZOOM_Elementor_Form {
	 ...
	 * Instance
	 *
	 * @var WPZOOM_Elementor_Form The single instance of the class.
	 * @since 1.0.0
	 * @access private
	 * @static
	 */
	...
	public function init_widgets() {
		...
		foreach ( $widgets as $path ) {
			...
			if ( file_exists( $file ) ) {
				require_once $file;
				$class_name = '\WPZOOMElementorForms\\' . ucwords( $slug_, '_' );
	...
	function add_widget_categories( $elements_manager ) {
		$elements_manager->add_category(
			'wpzoom-elementor-form',
			array(
				'title' => esc_html__( 'WPZOOM Forms', 'wpzoom-forms' ),
				'icon'  => 'fa fa-plug',
			)
		);
	}
